...
export default function FixFormatPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      <main className="w-full max-w-5xl mx-auto px-6 py-12 flex flex-col items-center">
        ...
        <div className="w-full max-w-3xl mb-8 grid gap-4 md:grid-cols-2">
          <button
            type="button"
            onClick={() => {
              setMode("format");
            ...
            className={`rounded-3xl border p-6 text-left transition-colors ${mode === "format" ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-white hover:border-slate-300"}`}
          >
            <Wand2 className="w-6 h-6 text-blue-600 mb-4" />
            <div className="font-black text-lg text-slate-900">Rapikan Cepat</div>
            <p className="mt-2 text-sm text-slate-500">Langsung format ulang DOCX: font 12/14, margin, heading, italic Inggris, tabel, gambar, dan daftar pustaka.</p>
          </button>
          <button
            type="button"
            onClick={() => {
            ...
            className={`rounded-3xl border p-6 text-left transition-colors ${mode === "import" ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-white hover:border-slate-300"}`}
          >
            <PenTool className="w-6 h-6 text-indigo-600 mb-4" />
            <div className="font-black text-lg text-slate-900">Masuk Editor</div>
            <p className="mt-2 text-sm text-slate-500">Deteksi struktur BAB/Sub-BAB dulu, lalu lanjut edit manual di editor terpadu.</p>
          </button>
        </div>
        ...
        <div className="w-full grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mb-12">
          <Card className="p-5 md:p-6 border-dashed bg-gray-50/50">
            {mode === "format" ? <Wand2 className="w-6 h-6 text-blue-500 mb-4" /> : <Search className="w-6 h-6 text-blue-500 mb-4" />}
            <h3 className="font-bold text-sm mb-2">{mode === "format" ? "Format Otomatis" : "Deteksi Otomatis"}</h3>
            <p className="text-xs text-gray-400">{mode === "format" ? "Rule dosen diterapkan langsung ke file DOCX asli." : "Kami mencari pola BAB I, 1.1, dan judul lainnya."}</p>
          </Card>
          <Card className="p-5 md:p-6 border-dashed bg-gray-50/50">
            <ArrowRight className="w-6 h-6 text-indigo-500 mb-4" />
            <h3 className="font-bold text-sm mb-2">{mode === "format" ? "Download Hasil" : "Konversi Struktur"}</h3>
        ...
        <div className="w-full max-w-2xl">
          {step === 1 && mode === "format" && (
            <FileUpload
              endpoint="/api/fix-format/format"
              ctaLabel="RAPIKAN DAN DOWNLOAD"
              successTitle="DOCX siap diunduh"
              successDescription="Dokumen sudah diproses sesuai aturan dasar dosen dan otomatis terdownload ke perangkat Anda."
            />
          )}
          ...
          {step === 1 && mode === "import" && (
            <FileUpload
              ...
              ctaLabel="IMPORT KE EDITOR"
              successTitle="Struktur berhasil dibaca"
              successDescription="Dokumen sudah siap dibuka ke mode editor terpadu."
            />
          )}
        ...
        <div className="mt-20 flex gap-4 items-center p-4 bg-gray-50 rounded-2xl border border-gray-100 max-w-2xl">
          <AlertCircle className="w-5 h-5 text-gray-400" />
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            Format yang didukung: .docx. Mode rapikan cepat fokus ke file siap print, sedangkan mode editor fokus ke struktur dokumen.
          </p>
        </div>
      </main>
    </div>
